...
const PaintingEdit = () => {
    ...
    useEffect(() => {
        const fetchPaintingData = async () => {
            try {
                const response = await axios.get(`/painting/one/${id}`);
                const painting = response.data;
                setName(painting.name);
                setDescription(painting.description);
                setFinishDate(painting.finishDate ? new Date(painting.finishDate).toISOString().split('T')[0] : '');
                setArtistId(painting.artistId);
                setGenreId(painting.genreId);
            } catch (error) {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const response = await axios.put(`/painting/${id}`, {
                name,
                description,
                finishDate,
                artistId,
            ...
            console.log('Painting updated:', response.data);
            navigate('/painting');
        } catch (error) {
        ...
        <div>
            ...
            <form className="form" onSubmit={handleSubmit}>
                ...
                <div className="container">
                    <button type="submit" className="add">Update Painting</button>
